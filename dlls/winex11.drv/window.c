...
static void set_size_hints( struct x11drv_win_data *data, DWORD style )
{
    ...
    if ((X11DRV_get_whole_window( GetAncestor( GetWindow( data->hwnd, GW_OWNER ), GA_ROOT ) ) && data->managed) ||
                                    ...
                                    struct window_surface *surface )
{
        ...
        ((new_style & WS_MINIMIZE) || is_window_rect_mapped( rectWindow )))
    {
        ...
        {
            set_wm_hints( data );
            HWND owner_win = X11DRV_get_whole_window( GetAncestor( GetWindow( data->hwnd, GW_OWNER ), GA_ROOT ));
